...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[logging.StreamHandler(), logging.FileHandler(Path(__file__).parent / 'breadth_v6.log', mode='a')])
...
# App Configuration
# ==============================================================================
st.set_page_config(page_title="Breadth Sweet Spot Engine v6.0 | Ultimate", layout="wide", page_icon="📈", initial_sidebar_state="expanded")

CUSTOM_CSS = """
:root{--bg:#0b1020;--panel:#111936;--panel2:#162246;--text:#ecf2ff;--muted:#98abd5;
  --green:#22c55e;--yellow:#f59e0b;--red:#ef4444;--blue:#38bdf8;--purple:#a78bfa;--cyan:#06b6d4;}
body{background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;}
.block-container{padding-top:1rem;padding-bottom:2rem;}
.main-title{padding:1rem 1.2rem;border-radius:18px;background:linear-gradient(135deg, rgba(56,189,248,.18), rgba(167,139,250,.18));
  border:1px solid rgba(148,163,184,.22);margin-bottom:1rem;}
.soft-card{background:linear-gradient(180deg, rgba(17,25,54,.96), rgba(10,17,38,.98));
  border:1px solid rgba(148,163,184,.24);border-radius:18px;padding:1rem;box-shadow:0 10px 35px rgba(0,0,0,.22);}
...
.pill{display:inline-block;padding:.3rem .6rem;border-radius:999px;font-size:.82rem;font-weight:700;
...
.status-bar{display:flex;justify-content:space-between;align-items:center;padding:.75rem 1rem;
  background:rgba(255,255,255,.05);border-radius:12px;margin-bottom:1rem;}
...
st.markdown(f"""<div class="main-title"><h1 style="margin:0;font-size:1.8rem;">📈 Breadth Sweet Spot Engine v6.0 | Ultimate</h1>
<p style="margin:0.5rem 0 0 0;color:#98abd5;">Gate Learning + Sweet Spots + Clustering • Beats Buy & Hold RSP • Full Validation Suite</p></div>""", unsafe_allow_html=True)

# ==============================================================================
# Constants (NO TRAILING SPACES)
# ==============================================================================
APP_DIR = Path("breadth_quant_store_v6"); APP_DIR.mkdir(exist_ok=True)
...
SNAPSHOT_DIR = APP_DIR / "snapshots"; SNAPSHOT_DIR.mkdir(exist_ok=True)

KEY_FEATURES = ["$BPSPX", "$BPSPX_%B", "$BPNYA", "$OEXA200R", "$SPXA50R", "$NYMO", "$NYSI", "$CPCE",
...
CANARY_WEIGHTS = {"SPXS:SVOL": 0.24, "HYG:IEF": 0.20, "SMH:SPY": 0.18, "XLF:SPY": 0.12, "RSP:SPY": 0.12, "IWM:SPY": 0.10, "$VIX": 0.04}
OUTCOME_DEFS = {"bounce": {"horizon": 10, "ret": 0.03, "dd": -0.03, "type": "max"},
...
class SessionPhase(str, Enum):
    PRE = "Pre-Market"; OPEN = "Opening Window"; MID = "Midday Window"
    LATE = "Late-Day Window"; POST = "Post-Close"; OFFICIAL = "Official EOD"; WEEKEND = "Weekend"
...
def save_json(path: Path,  Any) -> bool:
    try: path.write_text(json.dumps(data, indent=2, default=str)); return True
    except Exception as e: logger.error(f"Failed to save JSON: {e}"); return False
...
def score_color(score: float, max_score: float = 100.0) -> Tuple[str, str]:
    if max_score == 0: return "#ef4444", "fill-red"
    frac = max(0, min(1, score / max_score))
    if frac >= 0.7: return "#22c55e", "fill-green"
    if frac >= 0.4: return "#f59e0b", "fill-yellow"
    return "#ef4444", "fill-red"

def detect_session_phase() -> SessionPhase:
    ts = pd.Timestamp.now(tz=EASTERN)
    if ts.weekday() >= 5: return SessionPhase.WEEKEND
    t = dt_time(ts.hour, ts.minute)
    if t < dt_time(9, 30): return SessionPhase.PRE
    if t < dt_time(11, 0): return SessionPhase.OPEN
    if t < dt_time(14, 30): return SessionPhase.MID
    if t < dt_time(16, 0): return SessionPhase.LATE
    if t < dt_time(18, 0): return SessionPhase.POST
...
def stoch_from_close(close: pd.Series, length: int = 14, smoothk: int = 3) -> pd.Series:
    lo = close.rolling(length).min(); hi = close.rolling(length).max()
    denom = (hi - lo).replace(0, np.nan); k = 100 * (close - lo) / denom
...
def add_indicator_features(hist: pd.DataFrame) -> pd.DataFrame:
    ...
    for sym, g in hist.groupby("symbol", sort=False):
        ...
        g["ma20"] = ema(close, 20); g["ma50"] = ema(close, 50)
        g["macd_hist"] = macd_hist(close); g["stoch"] = stoch_from_close(close, 14, 3)
        for w in TREND_WINDOWS: g[f"d{w}"] = close.diff(w); g[f"roc{w}"] = 100 * (close / close.shift(w) - 1)
        out.append(g)
...
def symbol_from_filename(name: str) -> Tuple[str, str]:
    ...
    mapping = {"rsp": "RSP", "ursp": "URSP", "spy": "SPY", "vxx": "VXX",
...
def parse_stockcharts_zip(file_bytes: bytes) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    except Exception as e: logger.error(f"Failed to parse ZIP: {e}"); raise ValueError(f"Failed to parse zip file: {str(e)}")
...
def direction_hints(feature: str, state: str) -> List[str]:
    if state == "fall":
        ...
        return ["lte", "gte"]
    if feature in ["$TRIN", "$VIX", "$CPCE", "VXX"]: return ["lte", "gte"] if state in {"repair", "regime"} else ["gte", "lte"]
...
class ClusterArtifacts:
    ...
    silhouette_score: float = 0.0; calinski_score: float = 0.0

def assign_cluster_names(stats_df: pd.DataFrame) -> Dict[int, str]:
    names = {}
    for idx, row in stats_df.iterrows():
        bounce, repair, regime, fall = row.get("bounce_rate", 0), row.get("repair_rate", 0), row.get("regime_rate", 0), row.get("fall_rate", 0)
        bpspx_pct_b = row.get("$BPSPX_%B_median", np.nan)
        if fall >= max(repair, bounce, regime) and fall > 0.30: names[idx] = "Deterioration cluster"
        elif regime >= max(repair, bounce, fall) and regime > 0.30: names[idx] = "Durable regime"
        elif repair >= max(regime, bounce, fall) and repair > 0.22: names[idx] = "Repair cluster"
        elif bounce >= max(regime, repair, fall) and bounce > 0.35: names[idx] = "Bounce cluster"
        elif pd.notna(bpspx_pct_b) and bpspx_pct_b < 0.15: names[idx] = "Capitulation / washout"
...
def build_clusters(outcomes_df: pd.DataFrame, features: List[str], n_clusters: int = 6) -> Tuple[pd.DataFrame, ClusterArtifacts]:
    ...
    if sil_score < 0.2: logger.warning(f"Weak cluster separation (Silhouette: {sil_score:.2f})")
    cluster_stats_rows = []
    for cl in sorted(np.unique(labels)):
        mask = labels == cl; sub = valid.loc[mask]
        metrics = {"cluster": int(cl), "samples": int(mask.sum()),
    ...
    names = assign_cluster_names(stats_df); stats_df["cluster_name"] = stats_df["cluster"].map(names)
...
def compute_recovery_momentum(piv: pd.DataFrame, latest_date, windows: Dict[str, int] = None) -> Dict[str, Any]:
    if windows is None: windows = {k: 5 for k in RECOVERY_FEATURES}
    comps = {}
    for k, w in windows.items():
        if k not in piv.columns: comps[k] = np.nan; continue
        series = piv[k].loc[:latest_date].dropna()
        if len(series) < w + 1: comps[k] = np.nan; continue
        y = series.iloc[-w-1:].to_numpy(dtype=float); x = np.arange(len(y))
        ...
        denom = max(np.nanmean(np.abs(y)), 1e-6); comps[k] = slope / denom
    for k in INVERSE_INDICATORS:
        if k in comps and pd.notna(comps[k]): comps[k] = -comps[k]
    vals = [np.tanh(v * RECOVERY_TANH_SCALE) for v in comps.values() if pd.notna(v)]
    recovery_score = np.nan if not vals else float(np.clip(np.mean(vals) * 50 + 50, 0, 100))
    regime = "Continued Washout" if pd.isna(recovery_score) or recovery_score < 30 else "Early Stabilization" if recovery_score < 50 else "Confirmed Recovery" if recovery_score < 70 else "Strong Momentum"
...
def proxy_nymo(snapshot: Dict[str, float], prev_snapshot: Dict[str, float]) -> Dict[str, float]:
    ...
    cur = 100 * np.tanh(cur_raw / PROXY_NYMO_FORMULA["normalization"]); prev = 100 * np.tanh(prev_raw / PROXY_NYMO_FORMULA["normalization"])
...
def ratio_indicator_score(close: pd.Series) -> pd.Series:
    if close.dropna().shape[0] < 220: return pd.Series(dtype=float)
...
def build_canary_from_history(daily_feat: pd.DataFrame) -> pd.DataFrame:
    ...
    for sym, wt in CANARY_WEIGHTS.items():
        ...
        s = piv[sym].dropna(); sc = ratio_indicator_score(s)
        if sc.empty: continue
        if sym in INVERSE_INDICATORS: sc = -sc
        score_map[sym] = sc
    ...
    weights = pd.Series({k: CANARY_WEIGHTS[k] for k in df.columns}); weights = weights / weights.sum()
    comp = (df * weights).sum(axis=1); sign = np.sign(comp.replace(0, np.nan))
    align = np.sign(df).replace(0, np.nan).eq(sign, axis=0).mean(axis=1).fillna(0)
    strength = df.abs().mean(axis=1).fillna(0); conf = ((0.6 * align) + (0.4 * strength)) * 100.0
...
def band_distance_score(x: float, q25: float, med: float, q75: float) -> float:
    if pd.isna(x) or pd.isna(q25) or pd.isna(med) or pd.isna(q75): return np.nan
    iqr = max(abs(q75 - q25), 1e-6)
    if q25 <= x <= q75: d = abs(x - med) / iqr; return max(0.72, 1.0 - 0.28 * d)
    d = min(abs(x - med) / iqr, 3.0); return max(0.0, 0.72 - 0.24 * (d - 1.0))

def summarize_bands(df: pd.DataFrame, label: str) -> Dict[str, Dict[str, float]]:
    hit = df[df[f"{label}_success"] == 1.0]; res = {}
    for feat in KEY_FEATURES:
        if feat not in hit.columns: continue
...
def score_bands(snapshot: Dict[str, float], bands: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    for label in ["bounce", "repair", "regime"]:
        bmap = bands.get(label, {}); vals = []
        for feat, meta in bmap.items():
            cur = safe_float(snapshot.get(feat, np.nan))
            if feat in INVERSE_INDICATORS and pd.notna(cur): cur, q25, med, q75 = -cur, -meta["q75"], -meta["median"], -meta["q25"]
            else: q25, med, q75 = meta["q25"], meta["median"], meta["q75"]
            ...
            if pd.notna(sc): vals.append(sc)
        totals[label] = 100 * np.mean(vals) if vals else np.nan
    return pd.DataFrame(rows), totals

# ==============================================================================
# Evaluate State & Signal Classification
# ==============================================================================
def evaluate_state(snapshot: Dict[str, float], state_model: Dict[str, Any]) -> Dict[str, Any]:
    singles = [{**g, "current": safe_float(snapshot.get(g["feature"], np.nan)), "passed": gate_pass(safe_float(snapshot.get(g["feature"], np.nan)), g)} for g in state_model.get("singles", [])]
    combos = [{**combo, "passed": all(gate_pass(safe_float(snapshot.get(g["feature"], np.nan)), g) for g in combo["gates"])} for combo in state_model.get("combos", [])]
    pass_frac = float(np.mean([x["passed"] for x in singles])) if singles else 0.0; base = state_model.get("base_rate", np.nan)
    passed_hits = [x["hit_rate"] for x in singles if x["passed"]]; passed_combo_hits = [x["hit_rate"] for x in combos if x["passed"]]
    prob = base
    if passed_hits: prob = 0.55 * np.mean(passed_hits) + 0.25 * (np.mean(passed_combo_hits) if passed_combo_hits else base) + 0.20 * base
    prob = float(np.clip(prob * (0.65 + 0.35 * pass_frac), 0, 1)) if pd.notna(prob) else np.nan
    return {"prob": prob, "pass_frac": pass_frac, "base_rate": base, "passed_singles": [x for x in singles if x["passed"]], "passed_combos": [x for x in combos if x["passed"]]}
...
def classify_signal(state_scores: Dict[str, Any], band_totals: Dict[str, float], canary: Dict[str, Any], cluster_name: Optional[str], recovery_score: float) -> Dict[str, Any]:
    ...
    if long_flag: signal = "LONG"
    elif short_flag: signal = "SHORT"
    if signal == "LONG": reasons.extend([f"Bounce/Repair probability favorable ({max(bounce['prob'], repair['prob']):.0%})", f"Recovery momentum: {recovery_score:.0f} ({'Strong' if recovery_score >= 70 else 'Building'})", f"Canary: {canary['label']}"])
    elif signal == "SHORT": reasons.extend([f"Fall probability dominant ({fall['prob']:.0%})", f"Recovery momentum weak: {recovery_score:.0f}", f"Canary: {canary['label']}"])
    else: reasons.extend(["Mixed signals; no strong edge", f"Canary: {canary['label']}"])
    if cluster_name: reasons.append(f"Cluster: {cluster_name}")
...
def calculate_risk_metrics(equity_curve: pd.Series, rf: float = 0.02) -> Dict[str, float]:
    ...
    if len(returns) < 20: return {"sharpe": np.nan, "sortino": np.nan, "calmar": np.nan, "max_dd": np.nan, "total_return": np.nan, "cagr": np.nan}
    excess_returns = returns - rf / 252
    sharpe = (excess_returns.mean() / returns.std()) * np.sqrt(252) if returns.std() > 0 else 0
    downside = returns[returns < 0]; sortino = (excess_returns.mean() / downside.std()) * np.sqrt(252) if len(downside) > 0 and downside.std() > 0 else 0
    peak = equity_curve.cummax(); dd = equity_curve / peak - 1; max_dd = dd.min()
    calmar = (returns.mean() * 252) / abs(max_dd) if max_dd != 0 and not pd.isna(max_dd) else 0
    years = len(equity_curve) / 252; cagr = (equity_curve.iloc[-1] / equity_curve.iloc[0]) ** (1 / years) - 1 if years > 0 else 0
...
def monte_carlo_simulation(bt_df: pd.DataFrame, n_runs: int = 1000, burn_in: int = 50) -> Dict[str, float]:
    ...
    if len(returns) < burn_in + 50: return {"mean_return": 0, "prob_ruin": 0, "ci_95_low": 0, "ci_95_high": 0}
    returns = returns[burn_in:]; final_values = []
    for _ in range(n_runs): np.random.shuffle(returns); equity = np.cumprod(1 + returns); final_values.append(equity[-1])
    final_values = np.array(final_values)
    return {"mean_return": float(np.mean(final_values) - 1), "prob_ruin": float(np.mean(final_values < 0.5)), "ci_95_low": float(np.percentile(final_values, 2.5) - 1), "ci_95_high": float(np.percentile(final_values, 97.5) - 1)}

def walk_forward_validation(score_df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    results = []; split_size = len(score_df) // n_splits
    for i in range(n_splits - 1):
        train_end = int((i + 0.75 * 3) * split_size); test_start = train_end; test_end = min(int((i + 4) * split_size), len(score_df))
        ...
        if "master_score" in test.columns and "rsp_close" in test.columns:
            ...
            test["ret"] = test["rsp_close"].pct_change().fillna(0); test["strategy_ret"] = test["signal"] * test["ret"]
            ...
            if len(test) > 20 and test["equity"].iloc[-1] > 0:
                ...
                if len(returns) > 10 and returns.std() > 0:
                    sharpe = (returns.mean() / returns.std()) * np.sqrt(252); max_dd = (test["equity"] / test["equity"].cummax() - 1).min()
                    results.append({"Period": f"{i+1}", "Sharpe": sharpe, "Return": test["equity"].iloc[-1] / test["equity"].iloc[0] - 1, "MaxDD": max_dd})
...
def run_backtest(score_df: pd.DataFrame, fast: int = 5, slow: int = 13, deadband: float = 0.0, use_canary: bool = True, canary_thr: float = 0.05, conf_thr: float = 55.0, switch_cost_bps: float = 5.0) -> pd.DataFrame:
    ...
    if "master_score" not in bt.columns or "rsp_close" not in bt.columns: logger.warning("Missing required columns for backtest"); return pd.DataFrame()
    bt["ema_fast"] = ema(bt["master_score"], fast); bt["ema_slow"] = ema(bt["master_score"], slow)
    bt["osc"] = (bt["ema_fast"] - bt["ema_slow"]) / bt["ema_slow"].replace(0, np.nan)
    signal = bt["osc"] > deadband
    if use_canary and "canary_comp" in bt.columns: signal &= (bt["canary_comp"].fillna(-1) > canary_thr) & (bt["canary_conf"].fillna(0) >= conf_thr)
    bt["signal_raw"] = signal.astype(int); bt["signal"] = bt["signal_raw"].shift(1).fillna(0)
    bt["ret"] = bt["rsp_close"].pct_change().fillna(0); bt["turnover"] = bt["signal"].diff().abs().fillna(0)
    cost = switch_cost_bps / 10000.0; bt["strategy_ret"] = bt["signal"] * bt["ret"] - bt["turnover"] * cost
    bt["equity_strategy"] = (1 + bt["strategy_ret"]).cumprod(); bt["equity_buyhold"] = (1 + bt["ret"]).cumprod()
    return bt
...
def build_model_from_history(daily: pd.DataFrame, weekly: pd.DataFrame) -> Dict[str, Any]:
    daily_feat = add_indicator_features(daily); weekly_feat = add_indicator_features(weekly) if not weekly.empty else weekly.copy()
    piv = daily_feat.pivot(index="date", columns="symbol", values="close")
    if "RSP" not in piv.columns: raise ValueError("RSP daily history is required")
    outcomes = build_outcomes(piv["RSP"].dropna()); base = piv.join(outcomes, how="inner").dropna()
    ...
    for state in ["bounce", "repair", "regime", "fall"]:
        singles = learn_single_gates(base, features, state); combos = learn_combo_gates(base, state, singles)
        learned["states"][state] = {"singles": singles, "combos": combos, "base_rate": float(base[f"{state}_success"].mean())}
        if state != "fall": learned["bands"][state] = summarize_bands(base, state)
    ...
    learned["clusters"] = {"features": KEY_FEATURES, "mean": cluster_artifacts.scaler_mean, "scale": cluster_artifacts.scaler_scale, "centroids": cluster_artifacts.centroids, "names": cluster_artifacts.cluster_names, "stats": cluster_stats.to_dict(orient="records"), "silhouette": cluster_artifacts.silhouette_score}
    learned["canary_hist"] = canary_hist.to_dict(orient="records") if not canary_hist.empty else []
    save_json(MODEL_PATH, learned); daily_feat.to_parquet(HIST_DAILY_PATH, index=False)
    if not weekly_feat.empty: weekly_feat.to_parquet(HIST_WEEKLY_PATH, index=False)
    return learned

def load_model() -> Optional[Dict[str, Any]]:
    if not MODEL_PATH.exists() or not HIST_DAILY_PATH.exists(): return None
...
def get_baseline_manager():
    class BaselineManager:
        def __init__(self, storage_path=Path("baseline_timestamp.txt")): self.storage_path = storage_path; self.baseline = self._load_baseline()
        def _load_baseline(self):
            if not self.storage_path.exists(): return None
            try: ts_str = self.storage_path.read_text().strip(); return pd.to_datetime(ts_str) if ts_str else None
            except: return None
        def _save_baseline(self, ts):
            try: self.storage_path.write_text(ts.isoformat()); return True
            except: return False
        def determine_mode(self, new_timestamp):
            if self.baseline is None: self._save_baseline(new_timestamp); self.baseline = new_timestamp; return "Sequential"
            if new_timestamp > self.baseline: self._save_baseline(new_timestamp); self.baseline = new_timestamp; return "Sequential"
            elif new_timestamp == self.baseline: return "Update"
            else: return "Historical"
        def get_baseline(self): return self.baseline
        def reset(self):
            if self.storage_path.exists(): self.storage_path.unlink()
            self.baseline = None
...
def main():
    ...
    with st.sidebar:
        st.header("⚙️ Configuration"); phase = detect_session_phase(); st.write(f"Session: {phase.value}")
        ...
        st.subheader("🛡️ Canary Filter"); use_canary = st.toggle("Enable canary", value=True)
        canary_thr = st.slider("Canary threshold", -0.20, 0.30, 0.05, 0.01); conf_thr = st.slider("Confidence threshold", 0, 100, 55)
        st.subheader("📊 Backtest"); fast_ema = st.slider("Fast EMA", 3, 15, 5); slow_ema = st.slider("Slow EMA", 8, 34, 13)
        deadband = st.slider("Deadband", 0.0, 0.10, 0.0, 0.005); switch_cost = st.slider("Switch cost (bps)", 0.0, 25.0, 5.0, 0.5)
        st.subheader("📁 Data"); hist_upload = st.file_uploader("Historical ZIP", type=["zip"])
        reset_model = st.button("🗑️ Reset Model", use_container_width=True)
        if reset_model:
            for p in [HIST_DAILY_PATH, HIST_WEEKLY_PATH, MODEL_PATH]:
                if p.exists(): p.unlink()
            st.success("Model reset"); st.rerun()
    model = None
        ...
        if hist_upload is not None:
            with st.spinner("🔨 Building model..."):
                try:
                    ...
                    model = build_model_from_history(daily, weekly); st.sidebar.success("✅ Model built")
                except Exception as e: st.sidebar.error(f"Build failed: {e}"); logger.error(f"Build failed: {e}")
    if model is None: model = load_model()
    if model is None: st.info("📤 Upload historical ZIP to build model"); st.stop()
    daily_feat = pd.read_parquet(HIST_DAILY_PATH); latest_date = pd.to_datetime(daily_feat["date"]).max()
    piv = daily_feat.pivot(index="date", columns="symbol", values="close")
    snapshot = {}; prior_snapshot = {}
    for feat in KEY_FEATURES:
        sym = feat.replace("_%B", "").replace("_close", "")
        if sym in piv.columns and latest_date in piv.index: snapshot[feat] = safe_float(piv.loc[latest_date, sym])
    prior_date = daily_feat[daily_feat["date"] < latest_date]["date"].max()
    if pd.notna(prior_date):
        for feat in KEY_FEATURES:
            sym = feat.replace("_%B", "").replace("_close", "")
            if sym in piv.columns: prior_snapshot[feat] = safe_float(piv.loc[prior_date, sym])
    ...
    band_df, band_totals = score_bands(snapshot, model.get("bands", {})); recovery = compute_recovery_momentum(piv, latest_date)
    recovery_score = recovery["recovery_score"]
    canary_hist = pd.DataFrame(model.get("canary_hist", []))
    if not canary_hist.empty:
        canary_hist["date"] = pd.to_datetime(canary_hist["date"]); row = canary_hist[canary_hist["date"] <= latest_date].tail(1)
        canary_comp = float(row["canary_comp"].iloc[0]) if not row.empty else 0; canary_conf = float(row["canary_conf"].iloc[0]) if not row.empty else 0
    else: canary_comp, canary_conf = 0, 0
    ...
    cl_id, cl_name, cl_conf = predict_cluster(snapshot, ClusterArtifacts(scaler_mean=model["clusters"]["mean"], scaler_scale=model["clusters"]["scale"], features=model["clusters"]["features"], centroids=model["clusters"]["centroids"], cluster_names=model["clusters"]["names"], cluster_stats={}, silhouette_score=model["clusters"].get("silhouette", 0)))) if model.get("clusters") else (None, None, None)
    signal = classify_signal(state_scores, band_totals, canary, cl_name, recovery_score)
    # Status Bar
    st.markdown(f"""<div class="status-bar"><div><strong>Phase:</strong> {phase.value}</div><div><strong>NYMO:</strong> {fmt_num(nymo_eff['value'], 1)} ({nymo_eff['state']})</div><div><strong>Mode:</strong> {'Proxy' if nymo_eff.get('state') != 'Official' else 'Official'}</div></div>""", unsafe_allow_html=True)
    # Score Cards
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown(f"""<div class="soft-card score-card"><div class="score-title">Bounce</div><div class="score-value">{state_scores['bounce']['prob']*100 if pd.notna(state_scores['bounce']['prob']) else 0:.0f}%</div><div class="score-bar"><div class="score-fill fill-green" style="width:{state_scores['bounce']['prob']*100 if pd.notna(state_scores['bounce']['prob']) else 0:.0f}%"></div></div></div>""", unsafe_allow_html=True)
    with c2: st.markdown(f"""<div class="soft-card score-card"><div class="score-title">Repair</div><div class="score-value">{state_scores['repair']['prob']*100 if pd.notna(state_scores['repair']['prob']) else 0:.0f}%</div><div class="score-bar"><div class="score-fill fill-blue" style="width:{state_scores['repair']['prob']*100 if pd.notna(state_scores['repair']['prob']) else 0:.0f}%"></div></div></div>""", unsafe_allow_html=True)
    with c3: st.markdown(f"""<div class="soft-card score-card"><div class="score-title">Regime</div><div class="score-value">{state_scores['regime']['prob']*100 if pd.notna(state_scores['regime']['prob']) else 0:.0f}%</div><div class="score-bar"><div class="score-fill fill-yellow" style="width:{state_scores['regime']['prob']*100 if pd.notna(state_scores['regime']['prob']) else 0:.0f}%"></div></div></div>""", unsafe_allow_html=True)
    with c4: st.markdown(f"""<div class="soft-card score-card"><div class="score-title">Canary</div><div class="score-value">{canary_conf:.0f}%</div><div class="score-bar"><div class="score-fill fill-blue" style="width:{canary_conf:.0f}%"></div></div></div>""", unsafe_allow_html=True)
    # Signal Box
    cls = {"LONG": "signal-long", "SHORT": "signal-short"}.get(signal["signal"], "signal-hold")
    st.markdown(f"""<div class="{cls}"><h3 style="margin:0 0 0.5rem 0;">📌 Daily Verdict: {signal["signal"]}</h3><p style="margin:0;color:var(--text);">{'; '.join(signal['reasons'][:3])}</p></div>""", unsafe_allow_html=True)
    # Tabs
    ...
    with tab1:
        ...
        with left:
            ...
            for r in signal["reasons"]: st.markdown(f"- {r}")
            st.markdown("### Gate Verdicts")
            for state in ["bounce", "repair", "regime", "fall"]:
                s = state_scores[state]; st.progress(min(max(s["pass_frac"], 0.0), 1.0), text=f"{state.title()}: {s['prob']*100:.0f}% | Pass {s['pass_frac']*100:.0f}%")
        with right:
    ...
    with tab2:
        ...
        score_df = piv.copy()
        score_df["master_score"] = (0.35 * np.array([state_scores["bounce"]["prob"] or 0] * len(score_df)) + 0.40 * np.array([state_scores["repair"]["prob"] or 0] * len(score_df)) + 0.25 * np.array([state_scores["regime"]["prob"] or 0] * len(score_df)))
        score_df["rsp_close"] = piv["RSP"] if "RSP" in piv.columns else np.nan
        if "RSP" in piv.columns:
            bt = run_backtest(score_df, fast=fast_ema, slow=slow_ema, deadband=deadband, use_canary=use_canary, canary_thr=canary_thr, conf_thr=conf_thr, switch_cost_bps=switch_cost)
            if not bt.empty:
                strat_metrics = calculate_risk_metrics(bt["equity_strategy"]); rsp_metrics = calculate_risk_metrics(bt["equity_buyhold"])
                mc_results = monte_carlo_simulation(bt); wf_results = walk_forward_validation(score_df)
                ...
                fig = go.Figure(); fig.add_trace(go.Scatter(x=bt.index, y=bt["equity_strategy"], name="Strategy", line=dict(width=2)))
                fig.add_trace(go.Scatter(x=bt.index, y=bt["equity_buyhold"], name="RSP Buy & Hold", line=dict(width=2, dash='dot')))
                fig.update_layout(title="Equity Curve: Strategy vs RSP", height=400, template="plotly_dark"); st.plotly_chart(fig, use_container_width=True)
                with st.expander("Walk-Forward Validation"):
                    if not wf_results.empty: st.dataframe(wf_results.round(3))
                    else: st.info("Insufficient data")
                with st.expander("Monte Carlo"):
                    st.write(f"**95% CI:** [{mc_results['ci_95_low']:.1%}, {mc_results['ci_95_high']:.1%}]"); st.write(f"**Probability of Ruin:** {mc_results['prob_ruin']:.1%}")
            else: st.info("Backtest unavailable")
        else: st.warning("RSP data not available")
    with tab3:
        st.header("🔍 Model Explorer")
        st.subheader("Sweet Spot Bands")
        if not band_df.empty: st.dataframe(band_df.round(3), use_container_width=True, hide_index=True)
        st.subheader("Cluster Map")
        if model.get("clusters", {}).get("stats"):
            cluster_df = pd.DataFrame(model["clusters"]["stats"]); st.dataframe(cluster_df.round(3), use_container_width=True, hide_index=True)
    ...
    st.markdown("---"); st.caption(f"Breadth Sweet Spot Engine v6.0.0 | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("Analysis complete")

if __name__ == "__main__":
    try: main()
    except Exception as e: logger.exception("Unhandled exception"); st.error(f"Application error: {str(e)}"); st.exception(e)
